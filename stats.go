...
func GetRankedStats(summonerId int64, season string) (RankedStats, error) {
	args := createArgs("season", season)
	url := createApiUrl(STATS_RANKED) + fmt.Sprintf("%d/ranked?%v", summonerId, args)
...
func GetStatsSummary(summonerId int64, season string) (PlayerStatsSummaryList, error) {
	args := createArgs("season", season)
	url := createApiUrl(STATS_SUMMARY) + fmt.Sprintf("%d/summary?%v", summonerId, args)
